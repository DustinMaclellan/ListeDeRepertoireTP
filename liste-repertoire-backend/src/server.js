...
app.get("/api/demandes/:nomClient", (requete, reponse) => {
  ...
  utiliserDB(async (db) => {
    const listeDemandesClient = await db
      .collection("demandes")
      .find({ nomClient: nomClient })
      .toArray();
...
app.put("/api/demandes/inactive/:id", (requete, reponse) => {
  ...
  utiliserDB(async (db) => {
    var objectId = ObjectID.createFromHexString(id);
    const verificationActif = await db
      .collection("demandes")
      .findOne({ _id: objectId });
...
app.post("/api/utilisateur/verification", (requete, reponse) => {
  ...
  if (nomUtilisateur !== undefined || motDePasse !== undefined) {
    utiliserDB(async (db) => {
      const verificationUtilisateur = await db
        .collection("utilisateurs")
        .findOne({ nomUtilisateur: nomUtilisateur, motDePasse: motDePasse });
